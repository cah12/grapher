...
(function () {
  ...
  function LMnoMathMLNote() {
    ...
    nd.appendChild(an);
    nd.appendChild(
      document.createTextNode(" notation use Internet Explorer 6+")
    );
  ...
  function LMgetSymbol(str) {
    ...
    for (var i = 1; i <= str.length && more; i++) {
      ...
      more =
        k < LMnames.length && str.slice(0, LMnames[k].length) >= LMnames[k];
    }
  ...
  function LMparseSexpr(str) {
    ...
    switch (symbol.ttype) {
      ...
      case UNARY:
        ...
        if (symbol.input == "\\sqrt") {
          ...
          if (isIE) {
            ...
            return [
              LMcreateMmlNode(symbol.tag, result[0]),
              result[1],
              symbol.tag,
            ];
            ...
            for (i = 0; i < result[0].childNodes.length; i++)
              if (
                ...
                if (result[0].nodeName == "mi")
                  result[0] = LMcreateElementMathML("mo").appendChild(
      ...
      case BINARY:
        ...
        if (result[0] == null)
          return [
            LMcreateMmlNode("mo", document.createTextNode(symbol.input)),
            str,
            null,
          ];
      ...
      case INFIX:
        str = LMremoveCharsAndBlanks(str, symbol.input.length);
        return [
          LMcreateMmlNode("mo", document.createTextNode(symbol.output)),
          str,
          symbol.tag,
        ];
      default:
  ...
  function LMparseIexpr(str) {
    ...
    if (symbol.ttype == INFIX) {
      ...
      if (symbol.input == "_" || symbol.input == "^") {
        ...
        node.appendChild(result[0]);
        if (symbol.input == "\\choose") node = LMcreateMmlNode("mfenced", node);
      }
    }
    return [node, str, tag];
  }
  ...
  function LMprocessNode(n, linebreaks, spanclassLM) {
    ...
    if (spanclassLM != null) {
      ...
      for (var i = 0; i < frag.length; i++)
        if (frag[i].className == "LM") LMprocessNodeR(frag[i], linebreaks);
    } else {
      try {
        st = n.innerHTML;
      } catch (err) {}
      // DELIMITERS:
      if (st == null || st.indexOf("$") != -1) LMprocessNodeR(n, linebreaks);
    }
    if (isIE) {
