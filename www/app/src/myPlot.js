  ...
  constructor(plotDiv, plotTitle) {
    ...
    this.doNumerical = async function (fnDlg) {
      ...
      try {
        ...
        //   fnDlg.numOfPoints,
        // );
        const arrSides = fnDlg.numerical_fallbackFn.split("=");
        let fn = arrSides[0];
        ...
        for (let i = 0; i < branches.length; i++) {
          ...
          if (brn && brn.length > 3) {
            if (brn[0][0] !== fnDlg.lowerLimit) {
              const bndary = closerBoundary(fn, brn[0], brn[1], true);
              if (bndary) {
                brn.unshift(bndary);
              }
            }
            if (brn[brn.length - 1][0] !== fnDlg.upperLimit) {
              const bndary = closerBoundary(
                fn,
                brn[brn.length - 1],
                brn[brn.length - 2],
                false,
    ...
    this.functionDlgCb = async function (
      ...
      if (self._functionDlg.unboundedRange) {
        ...
        newCurve.discontinuityY = makeSamplesData.discontinuityY;
    ...
    const fsOptions = {
      ...
      fsServerUrl: "https://easy-grapher-ex.onrender.com", //only necessary for cross domain
