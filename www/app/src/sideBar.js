...
class SideBar {
  constructor(parent, plotDiv, gridItems, anchorPosition) {
    ...
    this.setGridItemHeigth = function () {
      //divide available height evenly
    ...
    this.gridItem = function (gridIndex) {
      if (gridIndex < gridItemsList.length) return gridItemsList[gridIndex];
      return null;
    };
    ...
    this.getIndexFromHeaderElement = function (headerElement) {
      for (var i = 0; i < self.gridItemCount(); ++i) {
        if (
          self.gridItem(i).headerElement[0].offsetTop ==
    ...
    Static.bind("resize", function () {
      ...
      //self.html().css("width", parseFloat($(".plotContainer").css("left")));
      //   return false;
      // }
    ...
    this.sidebarOriginalWidth = parseFloat(self.html().css("width"));
    //console.log(this.sidebarOriginalWidth);
  ...
  /* showSidebar(on) {
    ...
    if (on) {
      ...
      this.html().hide();
      const deltaWidth =
        parseFloat(this.html().css("width")) - this.sidebarOriginalWidth;
      this.plotDiv.css(
        "width",
        parseFloat(this.plotDiv.css("width")) + deltaWidth
      );
      this.html().css("width", this.sidebarOriginalWidth);
    }
    var plotDivWidthAsPercentage =
      (parseFloat(this.plotDiv.css("width")) /
        parseFloat(this.plotDiv.parent().css("width"))) *
      100;
    if (on) {
      if (this.anchorPosition() == "right") {
        this.plotDiv.css(
          "width",
          plotDivWidthAsPercentage - this.sideBarWidth() + "%"
        );
      } else if (this.anchorPosition() == "left") {
