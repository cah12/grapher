  ...
  constructor(plot, cb) {
    ...
    this.mouseUp = function (event) {
      ...
      if (this.moved) {
        ...
        //DOCS Draging a different curve prevents previous Undo Redo operation on a previous curve.
        if (
          this.pointDragStack.length &&
          this.pointDragStack[0].curve !== curve
        ) {
          self.pointDragStack = [];
          self.dragStartPoint = null;
          self.dragIndex = -1;
        }
        if (this.dragIndex < 0) {
          this.pointDragStack.push({
            curve,
            samples: curve
              .data()
              .samples()
    ...
    if (event.type === "keydown") {
      if (event.ctrlKey) {
        ...
        if (event.keyCode === 90) {
          ...
          if (this.dragIndex > 0) {
            this.dragIndex--;
            const dragState = this.pointDragStack[this.dragIndex];
            if (!dragState) return;
            const { curve, samples } = dragState;
            const plot = curve.plot();
            if (!plot) return;
        ...
        if (event.keyCode === 89) {
          //console.log("redo");
          if (this.dragIndex < this.pointDragStack.length - 1) {
            this.dragIndex++;
            const dragState = this.pointDragStack[this.dragIndex];
            //this.pointDragStack.push(dragState);
            if (!dragState) return;
            const { curve, samples } = dragState;
            const plot = curve.plot();
            if (!plot) return;
...
class CurveClosestPoint {
  constructor(plot, cb) {
    ...
    Static.bind("pointsTableUpdated", function (e, curve) {
      const dragState =
        self.eventHandlingObject.pointDragStack[
          self.eventHandlingObject.dragIndex
        ];
      if (!dragState) return;
      if (dragState.curve === curve) {
        self.eventHandlingObject.pointDragStack = [];
        self.eventHandlingObject.dragStartPoint = null;
        self.eventHandlingObject.dragIndex = -1;
      }
    });
  }
  ...
  setEnabled(on) {
    ...
    if (on) {
      //DOCS Selecting P-Sel or Add/Remove Point reset the drag state. i.e a new sequence of undo redo is started.
      this.eventHandlingObject.pointDragStack = [];
      this.eventHandlingObject.dragStartPoint = null;
      this.eventHandlingObject.dragIndex = -1;
      this.cw.installEventFilter(this.eventHandlingObject);
    } else {
